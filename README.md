...
Tips: 本项目**yatori-go-android**是基于**yatori-go-console**项目，将核心逻辑使用`go-mobile`移植而成。
...
> 3、史山轻喷（Orz）。
>
> 4、运行环境要求：① 安卓手机 ② 安卓版本≥13（miniSDK=33）
>
...
| 平台           | 描述                     | 状态     |
|----------------|---------------------------| ----------- |
| 英华学堂 | 支持暴力模式（会被检测到） | 移植完成待验证 🚧 |
| 海旗科技 | 支持暴力模式 | 移植完成待验证 🚧 |
| 仓辉实训 | 支持暴力模式（套壳英华版本会被检测到） | 移植完成待验证 🚧 |
| 智慧职教（资源库） | 默认秒刷(目前只支持Cookie登录方式)  | 移植完成待验证 🚧 |
| 学习通 | 支持自动写章测、作业、考试。<br>支持多课程模式和多任务点模式。 | 移植完成待验证 🚧 |
...
> 本项目基于 AGPL-3.0-or-later 开源。使用、修改、分发或提供网络服务时，请遵守 AGPL 协议要求，不要闭源套壳、假冒或用于违法滥用。
> 
